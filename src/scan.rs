...
use crate::option::{Protocol, TargetInfo};
use crate::result::{
    Domain, DomainScanResult, HostInfo, HostScanResult, Node, NodeType, PingResult, PingStat,
    PortScanResult, ProbeStatus, TraceResult,
};
...
pub fn run_port_scan(
    ...
    let dst: NsHostInfo = NsHostInfo::new_with_ip_addr(opt.targets[0].ip_addr)
        .with_ports(opt.targets[0].ports.clone())
        .with_host_name(opt.targets[0].host_name.clone());
...
pub fn run_host_scan(
    ...
    for target in opt.targets {
        let dst: NsHostInfo = NsHostInfo::new_with_ip_addr(target.ip_addr)
            .with_ports(target.ports)
            .with_host_name(target.host_name);
...
pub async fn run_service_scan(opt: ScanOption, msg_tx: &mpsc::Sender<String>) -> PortScanResult {
    ...
    if ps_result.results.len() > 0 {
        ...
        let host_info = crate::result::HostInfo {
            ...
            host_name: if let Some(target) = opt.targets.first() {
                target.host_name.clone()
            } else {
                network::lookup_ip_addr(ip.to_string())
            },
...
pub async fn run_node_scan(opt: ScanOption, msg_tx: &mpsc::Sender<String>) -> HostScanResult {
    ...
    for host in hs_result.hosts {
        let host_info = HostInfo {
            ...
            host_name: if host.host_name.is_empty() {
                dns_map
                    .get(&host.ip_addr)
                    .unwrap_or(&String::new())
                    .to_string()
...
pub fn run_domain_scan(opt: ScanOption, msg_tx: &mpsc::Sender<String>) -> DomainScanResult {
    ...
    if opt.use_wordlist {
        ...
        let text = match data {
            Ok(content) => content,
            Err(e) => {
                panic!("Could not open or find file: {}", e);
            }
        };
