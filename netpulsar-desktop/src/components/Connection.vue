    ...
    <Card>
        <template #title> 
            <div class="flex justify-content-between">
                <div class="flex">
                    Active TCP connections and the TCP and UDP ports on which is listening. Click row for more detail.  
                </div>
                <div class="flex">
                    <ToggleButton v-model="autoUpdate" onLabel="Auto" offLabel="Manual" onIcon="pi pi-play" offIcon="pi pi-pause" class="mr-2" />
                    <Button type="button" icon="pi pi-refresh" outlined :loading="isLoading" @click="GetNetStat" />
                </div>
            </div>
        </template>
        <template #content>
            <DataTable :value="tableData" v-model:selection="selectedHost" :loading="isLoading" selectionMode="single" dataKey="index" @rowSelect="onRowSelect" @rowUnselect="onRowUnselect" size="small" scrollable scrollHeight="70vh" tableStyle="min-width: 50rem">
