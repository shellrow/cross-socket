...
pub const NETPULSAR_CONFIG_FILE_NAME: &str = "netpulsar-config.json";
...
impl AppConfig {
    ...
    pub fn load() -> AppConfig {
        match sys::get_user_file_path(NETPULSAR_CONFIG_FILE_NAME) {
            Some(path) => {
    ...
    pub fn save(&self) {
        if let Some(path) = sys::get_user_file_path(NETPULSAR_CONFIG_FILE_NAME) {
